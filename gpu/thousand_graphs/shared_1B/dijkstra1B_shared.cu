...
__global__ void relax(int *dist_d, int *parent_d, int *visited_d, int *adjMat_d, int u, int width)
{
    ...
    // Must use extern as the size of adjMat_ds is declared in parent kernel
    extern __shared__ int adjMat_ds[];

    // Bring the adjacency data from global memory to shared memory
    for(int i = 0; i < width; i++)
        adjMat_ds[i * width + tid_x] = adjMat_d[i * width + tid_x];
    __syncthreads();

    // Each thread will attempt to relax an adjacent vertex if an edge exists
    // If the vertex is unvisited and the edge exists
    if(tid_x != u && visited_d[tid_x] == 0 && adjMat_ds[offset] >= 0)
    {
        int v_dist = adjMat_ds[offset];
...
{   
    ...
    int shared_mem_size = width * width * sizeof(int);
